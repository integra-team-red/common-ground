...
export class FeedbackPage implements OnInit {
    ...
    loadFeedbacksLazy(event: TableLazyLoadEvent) {
        ...
        this.feedbackService.getAll({page, size}).subscribe(async (response: any) => {
            if (response instanceof Blob) {
                const text = await response.text();
                const data = JSON.parse(text);

                this.feedbacks.set(data.content ?? []);
    ...
    filteredFeedbacks = computed(() => {
        ...
        if (!query) return all;
        return all.filter(f => f.email?.toLowerCase().includes(query) || f.message?.toLowerCase().includes(query));
    });
}
